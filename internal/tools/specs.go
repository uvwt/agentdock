...
func allToolSpecs() []ToolSpec {
	return []ToolSpec{
		...
		{Name: "apply_patch", Title: "Update workspace files", Description: "Update workspace files using structured edits.", Profiles: unifiedProfiles(), Handler: ctxToolHandler((*Runtime).applyPatch)},
		{Name: "edit_file", Title: "Edit file", Description: "Replace exact UTF-8 text in one workspace file with match-count checks and diff preview.", Profiles: unifiedProfiles(), Handler: toolHandler((*Runtime).editFile)},
		...
		{Name: "workspace_repos", Title: "Workspace repositories", Description: "List Git repositories found under the workspace.", ReadOnly: true, Profiles: readOnlyProfiles(), Handler: ctxToolHandler((*Runtime).workspaceRepos)},
		{Name: "git_status", Title: "Git status", Description: "Return git working tree status for the workspace.", ReadOnly: true, Profiles: readOnlyProfiles(), Handler: ctxToolHandler((*Runtime).gitRepoStatus)},
		{Name: "git_diff", Title: "Git diff", Description: "Return unified git diff for workspace changes.", ReadOnly: true, Profiles: readOnlyProfiles(), Handler: ctxToolHandler((*Runtime).gitDiff)},
		{Name: "git_log", Title: "Git log", Description: "Return recent git commits with bounded structured metadata.", ReadOnly: true, Profiles: readOnlyProfiles(), Handler: ctxToolHandler((*Runtime).gitLog)},
		{Name: "git_inspect", Title: "Git inspect", Description: "Inspect Git history through one unified show/blame tool.", ReadOnly: true, Profiles: readOnlyProfiles(), Handler: ctxToolHandler((*Runtime).gitInspect)},
		{Name: "git_remote", Title: "Git remote", Description: "Run Git remote operations through one unified fetch/pull/push tool.", OpenWorld: true, Profiles: unifiedProfiles(), Handler: ctxToolHandler((*Runtime).gitRemote)},
