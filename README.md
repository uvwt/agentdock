...
- **动态 Plugin**：支持从 AgentDock 加载 `plugins` 动态能力，新增应用脚本不需要重新编译 MCP。
...
| 工具 | 说明 |
| --- | --- |
| `plugin_list` | 列出 workspace 中已安装的动态 plugin。 |
| `plugin_describe` | 查看指定 plugin 的说明、动作和输入 schema。 |
| `plugin_call` | 调用指定 plugin action，并传入结构化参数。 |
...
```text
/srv/agentdock/AgentDock/plugins/
```
...
AgentDock 可以在 macOS 上裸机运行，适合本地项目管理、Git 自动化、文件处理、动态 Plugin 和浏览器自动化测试。macOS 没有 Linux Landlock，也没有 systemd，所以它更适合本地开发/自动化，不建议当作生产部署服务器。
...
| `AGENTDOCK_DIR` | `AgentDock` | AgentDock 控制层目录。plugin、browser runner、artifacts 默认相对该目录解析。 |
| `AGENTDOCK_PLUGIN_DIR` | `plugins` | AgentDock-relative 动态 plugin 目录。 |
...
Plugin 用于把具体应用能力做成热插拔脚本，避免每新增一个场景都改 Go 代码、重建镜像和刷新 MCP 工具列表。
...
{
  "name": "hello",
  "description": "Example plugin that echoes structured input.",
  "version": "0.1.0",
  "actions": {
    "echo": {
      "description": "Echo PLUGIN_ARGS_JSON as JSON output.",
...
```text
plugin_list
plugin_describe(plugin="hello")
plugin_call(plugin="hello", action="echo", args={"message":"hi"})
```
...
| `PLUGIN_ACTION` | action 名称。 |
| `PLUGIN_ARGS_JSON` | 调用方传入的结构化参数 JSON。 |
| `WORKSPACE` | workspace 根目录。 |

注意事项：

- 当前 MVP 使用 `plugin.json`，暂不解析 YAML。
- plugin 目录在 AgentDock 内，默认是 `<AgentDock>/plugins`。
- `command` 在 plugin 目录下执行，建议使用相对路径调用 `scripts/` 下脚本。
- `output=json` 时，MCP 会尝试把 stdout 解析成 JSON 并放入返回的 `json` 字段。
- 如果 plugin 需要密钥，建议从服务环境变量读取，并在 `plugin.json` 的 `secrets` 中声明变量名，MCP 会在描述时只返回是否配置，不返回密钥内容。
- 新增/修改 plugin 文件不需要重启 MCP；修改 Go 核心代码或安装系统依赖才需要重新部署。
...
AGENTDOCK_MEMORY_TOKEN=optional-token
AGENTDOCK_MEMORY_TIMEOUT_MS=30000
```
...
| `memory_pack` | 按项目打包 shared profile、overview、conventions、environment、decisions、runbooks、handoff。 |
| `memory_append_note` | 向 MemoryDock 追加临时笔记，默认写入 `inbox/`。 |
...
AgentDock 是一个本地/远程 Agent 工具运行层，支持文件、Git、动态 plugin、浏览器和实验性桌面自动化。
```
