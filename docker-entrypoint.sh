...
mkdir -p "$AGENTDOCK_HOME_DIR/$AGENTDOCK_BROWSER_ARTIFACT_DIR"
...
if [ "${AGENTDOCK_BROWSER_ENABLED:-false}" = "true" ] && [ -d /opt/agentdock/browser-runner ]; then
  if [ ! -f "$AGENTDOCK_HOME_DIR/$AGENTDOCK_BROWSER_RUNNER_DIR/browser-runner.js" ]; then
    mkdir -p "$AGENTDOCK_HOME_DIR/$AGENTDOCK_BROWSER_RUNNER_DIR"
    cp -R /opt/agentdock/browser-runner/. "$AGENTDOCK_HOME_DIR/$AGENTDOCK_BROWSER_RUNNER_DIR/"
  fi
fi
