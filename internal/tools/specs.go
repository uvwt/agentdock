...
func readOnlyProfiles() []string { return []string{config.ProfileFull, config.ProfileReadOnly} }
func fullProfiles() []string     { return []string{config.ProfileFull} }
...
func allToolSpecs() []ToolSpec {
	...
	return bindToolSchemas([]ToolSpec{
		...
		{Name: "workspace_edit", Title: "Edit workspace", Description: "Edit workspace files through one action-based entrypoint: replace, patch, add, delete, or move.", Profiles: fullProfiles(), Handler: ctxToolHandler((*Runtime).workspaceEdit)},
		{Name: "exec_command", Title: "Run workspace command", Description: "Run a bounded command in the workspace with sandbox and approval controls.", OpenWorld: true, Profiles: fullProfiles(), Handler: ctxToolHandler((*Runtime).execCommand)},
		...
		{Name: "session_act", Title: "Act on command sessions", Description: "Write to or stop command sessions through a mutating session tool.", Destructive: true, Profiles: fullProfiles(), Handler: toolHandler((*Runtime).sessionAct)},
		...
		{Name: "task_manage", Title: "Manage recoverable tasks", Description: "Persist and resume substantial AgentDock tasks. Use workflow_template_manage action=match for template discovery.", Profiles: fullProfiles(), Handler: toolHandler((*Runtime).taskManage)},
		{Name: "workflow_template_manage", Title: "Manage workflow templates", Description: "List, get, save, validate, publish, retire, or match AgentDock workflow templates.", Profiles: fullProfiles(), Handler: toolHandler((*Runtime).workflowTemplateManage)},
		{Name: "skill_manage", Title: "Manage AgentDock Skills", Description: "List, inspect, validate, install, run, or roll back AgentDock Skills through the local Skill Runtime.", Destructive: true, OpenWorld: true, Profiles: fullProfiles(), Handler: ctxToolHandler((*Runtime).skillManage)},
		{Name: "env_manage", Title: "Manage Skill environment", Description: "Manage redacted Skill environment variables through the local Nexus Env Registry.", Destructive: true, OpenWorld: true, Profiles: fullProfiles(), Handler: ctxToolHandler((*Runtime).envManage)},
		...
		{Name: "git_write", Title: "Write Git repository state", Description: "Run mutating Git operations through one action-based entrypoint: clone, commit, fetch, pull, or push.", OpenWorld: true, Profiles: fullProfiles(), Handler: ctxToolHandler((*Runtime).gitWrite)},
		...
		{Name: "recall_write", Title: "Write RecallDock entry", Description: "Plan, create, replace, append, patch, update facts, diff, or delete RecallDock content. The model must choose target=card/note/markdown and action explicitly.", Destructive: true, OpenWorld: true, Profiles: fullProfiles(), Availability: requiresRecall, Handler: ctxToolHandler((*Runtime).recallWrite)},
		{Name: "recall_maintain", Title: "Maintain RecallDock", Description: "Run RecallDock maintenance actions such as sync_status, list, lint, embedding_status, reindex, or reindex_cards.", Destructive: true, OpenWorld: true, Profiles: fullProfiles(), Availability: requiresRecall, Handler: ctxToolHandler((*Runtime).recallMaintain)},
		...
		{Name: "private_notes_write", Title: "Write private note", Description: "Write a plaintext private note under private-notes/notes and always generate a mandatory age encrypted .md.age backup under private-notes/encrypted. Do not use recall_write for private material.", OpenWorld: true, Profiles: fullProfiles(), Handler: ctxToolHandler((*Runtime).privateNotesWrite)},
		{Name: "private_notes_status", Title: "Inspect private notes", Description: "List private notes or check whether mandatory age encrypted backups are present.", ReadOnly: true, OpenWorld: true, Profiles: readOnlyProfiles(), Handler: ctxToolHandler((*Runtime).privateNotesStatus)},
		{Name: "private_notes_maintain", Title: "Maintain private notes", Description: "Initialize age encryption, sync encrypted backups, or migrate legacy private-note backups.", Destructive: true, OpenWorld: true, Profiles: fullProfiles(), Handler: ctxToolHandler((*Runtime).privateNotesMaintain)},
		{Name: "browser_session", Title: "Browser session", Description: "Start, close, or clean up a browser automation session; supports persistent profiles and injected cookies/localStorage.", OpenWorld: true, Profiles: fullProfiles(), Availability: requiresBrowser, Handler: ctxToolHandler((*Runtime).browserSession)},
		{Name: "browser_act", Title: "Browser actions", Description: "Run browser automation actions and capture a structured screenshot snapshot; supports close_after, inline image, and storage state save.", OpenWorld: true, Profiles: fullProfiles(), Availability: requiresBrowser, Handler: func(ctx context.Context, r *Runtime, args map[string]any) (Result, error) {
		...
		{Name: "desktop_act", Title: "Act on desktop", Description: "Unified macOS desktop action tool for focus, move, click, double-click, scroll, drag, type, set value, accessibility actions, hotkeys, and waits.", Destructive: true, Profiles: fullProfiles(), Availability: requiresDesktop, Handler: ctxToolHandler((*Runtime).desktopAct)},
		{Name: "desktop_clipboard_read", Title: "Read desktop clipboard", Description: "Read macOS clipboard text through a read-only clipboard tool.", ReadOnly: true, Profiles: readOnlyProfiles(), Availability: requiresDesktop, Handler: ctxToolHandler((*Runtime).desktopClipboardRead)},
		{Name: "desktop_clipboard_write", Title: "Write desktop clipboard", Description: "Set macOS clipboard text through a mutating clipboard tool.", Destructive: true, Profiles: fullProfiles(), Availability: requiresDesktop, Handler: ctxToolHandler((*Runtime).desktopClipboardWrite)},
		{Name: "artifact_send", Title: "Send encrypted artifact", Description: "Encrypt and send a top-level file parameter or local file/directory through AgentDock Nexus to one or more registered devices. The target only writes to its controlled inbox or configured logical target and never executes the file.", OpenWorld: true, FileArgRewritePaths: []string{"file"}, Profiles: fullProfiles(), Availability: requiresNexus, Handler: ctxToolHandler((*Runtime).artifactSend)},
		{Name: "artifact_fetch_create", Title: "Create artifact fetch", Description: "Create an asynchronous high-risk request for a registered device to list or encrypt an absolute-path file or directory under immutable deny rules.", Destructive: true, OpenWorld: true, Profiles: fullProfiles(), Availability: requiresArtifactFetch, Handler: ctxToolHandler((*Runtime).artifactFetchCreate)},
		{Name: "artifact_fetch_status", Title: "Artifact fetch status", Description: "Return status or a bounded directory listing for a local artifact fetch request.", ReadOnly: true, OpenWorld: true, Profiles: fullProfiles(), Availability: requiresArtifactFetch, Handler: ctxToolHandler((*Runtime).artifactFetchStatus)},
		{Name: "artifact_fetch_download", Title: "Download artifact fetch", Description: "Download and decrypt a ready artifact fetch, return a file resource, or confirm that the GPT sandbox mounted it so ciphertext can be deleted.", Destructive: true, OpenWorld: true, FileResultRewritePaths: []string{"file_path"}, Profiles: fullProfiles(), Availability: requiresArtifactFetch, Handler: ctxToolHandler((*Runtime).artifactFetchDownload)},
	})
}
