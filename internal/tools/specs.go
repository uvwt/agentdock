...
func allToolSpecs() []ToolSpec {
	...
	return bindToolSchemas([]ToolSpec{
		...
		{Name: "skill_package", Title: "Manage Skill packages", Description: "Validate, install, or roll back AgentDock Skill packages through the local Skill store.", Handler: ctxToolHandler((*Runtime).skillPackage)},
		{Name: "mcp_manage", Title: "Manage dynamic MCP servers", Description: "Register, inspect, enable, disable, refresh, or remove dynamic MCP servers. Dynamic MCP tools remain separate from AgentDock built-in tools.", Handler: ctxToolHandler((*Runtime).mcpManage)},
