...
func allToolSpecs() []ToolSpec {
	...
	return bindToolSchemas([]ToolSpec{
		...
		{Name: "read_file", Title: "Read file", Description: "Read a UTF-8 text file slice. Supports normal Host paths and skill://<name>/<path> resources from the active Skill version.", Handler: toolHandler((*Runtime).readFile)},
		...
		{Name: "list_files", Title: "List files", Description: "List files using glob and ignore filters. Relative paths resolve from ~/AgentDock by default.", Handler: ctxToolHandler((*Runtime).listFiles)},
		...
		{Name: "exec_command", Title: "Run command", Description: "Run a bounded command. Optionally load one isolated Skill environment before applying explicit env overrides. Relative workdir values resolve from ~/AgentDock; actual access follows the Host path model.", Handler: ctxToolHandler((*Runtime).execCommand)},
		...
		{Name: "task_manage", Title: "Manage recoverable tasks", Description: "Persist substantial AgentDock tasks and update live step progress with checkpoint. Use workflow_template_manage action=match for template discovery.", Handler: ctxToolHandler((*Runtime).taskManage)},
		{Name: "workflow_template_manage", Title: "Manage workflow templates", Description: "List, get, get multiple, save, validate, publish, retire, or match AgentDock workflow templates. get_many requires the model to compose the returned templates before task creation.", Handler: ctxToolHandler((*Runtime).workflowTemplateManage)},
		{Name: "skill_package", Title: "Manage Skill packages", Description: "Validate, install, or roll back AgentDock Skill packages and manage each Skill's isolated environment without returning secret values.", Handler: ctxToolHandler((*Runtime).skillPackage)},
		...
		{Name: "browser_act", Title: "Browser actions", Description: "Run browser automation actions and return the final screenshot as a lightweight AgentDock Artifact reference; supports close_after and storage state save. If bundled Chromium is missing, retry with browser=chrome rather than suggesting the Playwright browser install command.", Availability: requiresBrowser, Handler: func(ctx context.Context, r *Runtime, args map[string]any) (Result, error) {
			return r.browserRunnerCall(ctx, "action", args)
		}},
		{Name: "browser_snapshot", Title: "Browser snapshot", Description: "Capture page state and return the screenshot as a lightweight AgentDock Artifact reference with page text, viewport, visible interactive elements, and browser errors. If bundled Chromium is missing, retry with browser=chrome rather than suggesting the Playwright browser install command.", Availability: requiresBrowser, Handler: func(ctx context.Context, r *Runtime, args map[string]any) (Result, error) {
