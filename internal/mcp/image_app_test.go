...
func TestDirectImageAppUsesSharedRendererAndPreservesPixels(t *testing.T) {
	...
	for _, enabled := range []bool{false, true} {
		...
		h := newMCPAppTestHarnessWithApps(t, config.Config{AgentDockDefaultDir: root, AgentDockHome: filepath.Join(root, ".agentdock")}, enabled)
		...
		if enabled {
			assertToolUIResource(t, found, protocol.ImageUIResourceURI, "openai/outputTemplate")
		} else if found.Meta["ui"] != nil {
			t.Fatal("Apps disabled")
		}
		result, err := h.session.CallTool(t.Context(), &mcpsdk.CallToolParams{Name: "view_image", Arguments: map[string]any{"path": path, "format": "png"}})
		...
		failed, err := h.session.CallTool(t.Context(), &mcpsdk.CallToolParams{Name: "view_image", Arguments: map[string]any{"path": filepath.Join(root, "missing.png")}})
