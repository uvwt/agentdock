...
func OutputSchema(name string) map[string]any {
	...
	case "file_publish":
		...
		props["size"] = intProp("Snapshot payload size in bytes.")
	...
	case "private_note_manage":
		...
		props["results"] = arrayProp("Search results with redacted snippets for action=search.")
		props["redacted"] = boolProp("Whether search snippets were redacted.")
		props["path"] = stringProp("Plain note path for read/write results.")
		props["encrypted_path"] = stringProp("Age encrypted backup path, typically encrypted/<category>/<name>.md.age.")
		props["content"] = stringProp("Plaintext content returned by action=read.")
		props["truncated"] = boolProp("Whether returned content was truncated.")
		props["contains_secret"] = boolProp("Whether the note content appears to contain secrets.")
