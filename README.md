...
| Goal Mode（实验） | 持久 Goal、Capsule、Reasoning Lease、结构化 commit_turn 与跨对话恢复；详见 docs/goal-mode.md |
| 面向正式部署 | 提供 Docker、systemd、macOS 和 Windows 安装方式，并持续发布正式镜像 |

## Goal Mode（產品）

長任務閉環：**ChatGPT 網頁 → MCP → 本地執行 → 摘要回貼 ChatGPT**。無操作台。

```bash
source ~/.agentdock/browser/env.sh
go run ./cmd/agentdock-desktop   # 打開 /console 設定台 + MCP + ChatGPT
```

說明：[docs/goal-mode-product.md](./docs/goal-mode-product.md)
