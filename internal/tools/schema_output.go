...
func OutputSchema(name string) map[string]any {
	...
	case "goal_manage":
		props["action"] = stringProp("Completed goal action.")
		props["goal_id"] = stringProp("Persistent goal id.")
		props["capsule"] = objectProp("Fixed-budget Goal Capsule for reasoning workers, including resume_prompt.")
		props["goal"] = objectProp("Full durable goal document when get is called with full=true.")
		props["goal_summary"] = objectProp("Compact goal summary for list and lifecycle actions.")
		props["goals"] = arrayProp("Compact goal summaries ordered by most recent update.")
		props["lease"] = objectProp("Reasoning lease returned by acquire_lease.")
		props["evidence"] = arrayProp("Evidence references returned by get_evidence or add_evidence.")
		props["count"] = intProp("Returned item count.")
		props["state_dir"] = stringProp("Local AgentDock goal state directory.")
		props["next_required_action"] = stringProp("Concise guidance for the next worker or user action.")
		props["verify"] = objectProp("Verifier report from verify or mark_completed preflight.")
		props["policy"] = objectProp("Policy decision from check_policy.")
		props["run"] = objectProp("Deterministic workflow/step execution result from execute_steps or run_workflow.")
		props["artifact"] = objectProp("Content-addressed artifact metadata from store_artifact.")
		props["active_goal_id"] = stringProp("Goal currently bound for high-risk tool policy gating.")
		props["previous_goal_id"] = stringProp("Previously bound goal id returned by unbind.")
