...
func TestCommitStagedPatchRollsBackEarlierFilesWhenInstallFails(t *testing.T) {
	...
	rename := func(source, target string) error {
		if !failed && strings.Contains(filepath.Base(source), ".agentdock-patch-write-") && target == second {
			failed = true
			return installFailure
		}
		return os.Rename(source, target)
	}
	if err := commitStagedPatchWithRename(staged, rename); !errors.Is(err, installFailure) {
		t.Fatalf("commit error = %v, want simulated install failure", err)
	}
	for _, path := range []string{first, second} {
