...
func (svc *Service) notesSearch(ctx context.Context, args map[string]any) (Result, error) {
	...
	indexFound := false
	if index, err := svc.memoryRead(ctx, map[string]any{"path": scope.IndexPath}); err == nil {
		indexFound = true
	...
	searchResults := []any{}
	if result, err := svc.memorySearch(ctx, map[string]any{"query": query, "prefix": scope.Prefix, "max_results": maxResults}); err == nil {
...
func (svc *Service) notesCapture(ctx context.Context, args map[string]any) (Result, error) {
	...
	search, err := svc.notesSearch(ctx, map[string]any{"scope": scope.Name, "query": question, "max_results": intArg(args, "max_results", 5)})
	if err != nil {
		return nil, err
	}
...
func (svc *Service) notesWrite(ctx context.Context, args map[string]any) (Result, error) {
