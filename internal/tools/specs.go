...
func allToolSpecs() []ToolSpec {
	...
	return []ToolSpec{
		...
		{Name: "workspace_edit", Title: "Edit workspace", Description: "Edit workspace files through one action-based entrypoint. Recommended actions: replace or patch. Legacy apply_patch/edit_file wrappers return replacement_tool metadata.", Profiles: unifiedProfiles(), Handler: ctxToolHandler((*Runtime).workspaceEdit)},
		{Name: "apply_patch", Title: "Update workspace files", Description: "Deprecated compatibility wrapper for workspace_edit action=patch.", Profiles: unifiedProfiles(), Handler: ctxToolHandler((*Runtime).applyPatchCompat)},
		{Name: "edit_file", Title: "Edit file", Description: "Deprecated compatibility wrapper for workspace_edit action=replace.", Profiles: unifiedProfiles(), Handler: toolHandler((*Runtime).editFileCompat)},
		...
		{Name: "task_manage", Title: "Manage recoverable tasks", Description: "Persist and resume substantial AgentDock tasks with create/list/get/block/resume/final_review/complete_after_review plus template_match.", Profiles: unifiedProfiles(), Handler: toolHandler((*Runtime).taskManage)},
		...
		{Name: "workspace_repos", Title: "Workspace repositories", Description: "Deprecated compatibility wrapper for git_read action=repos.", ReadOnly: true, Profiles: readOnlyProfiles(), Handler: ctxToolHandler((*Runtime).workspaceReposCompat)},
		{Name: "git_status", Title: "Git status", Description: "Deprecated compatibility wrapper for git_read action=status.", ReadOnly: true, Profiles: readOnlyProfiles(), Handler: ctxToolHandler((*Runtime).gitStatusCompat)},
		{Name: "git_diff", Title: "Git diff", Description: "Deprecated compatibility wrapper for git_read action=diff.", ReadOnly: true, Profiles: readOnlyProfiles(), Handler: ctxToolHandler((*Runtime).gitDiffCompat)},
		{Name: "git_log", Title: "Git log", Description: "Deprecated compatibility wrapper for git_read action=log.", ReadOnly: true, Profiles: readOnlyProfiles(), Handler: ctxToolHandler((*Runtime).gitLogCompat)},
		{Name: "git_inspect", Title: "Git inspect", Description: "Deprecated compatibility wrapper for git_read action=show/blame.", ReadOnly: true, Profiles: readOnlyProfiles(), Handler: ctxToolHandler((*Runtime).gitInspectCompat)},
		{Name: "git_remote", Title: "Git remote", Description: "Deprecated compatibility wrapper for git_write action=fetch/pull/push.", OpenWorld: true, Profiles: unifiedProfiles(), Handler: ctxToolHandler((*Runtime).gitRemoteCompat)},
		{Name: "git_clone", Title: "Git clone", Description: "Deprecated compatibility wrapper for git_write action=clone.", OpenWorld: true, Profiles: unifiedProfiles(), Handler: ctxToolHandler((*Runtime).gitCloneCompat)},
		{Name: "git_commit", Title: "Git commit", Description: "Deprecated compatibility wrapper for git_write action=commit.", Profiles: unifiedProfiles(), Handler: ctxToolHandler((*Runtime).gitCommitCompat)},
		...
		{Name: "recall_write", Title: "Write RecallDock entry", Description: "Plan, write, patch, update facts, or delete RecallDock content. The model must choose kind=card, note, markdown, patch, fact, delete, or explicit auto for a non-writing plan.", Destructive: true, OpenWorld: true, Profiles: unifiedProfiles(), Availability: requiresRecall, Handler: ctxToolHandler((*Runtime).recallWrite)},
		...
		{Name: "browser_profile", Title: "Browser profile", Description: "Open, inspect, or close a managed browser profile.", OpenWorld: true, Profiles: unifiedProfiles(), Availability: requiresBrowser, Handler: ctxToolHandler((*Runtime).browserProfile)},
