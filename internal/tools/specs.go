...
func allToolSpecs() []ToolSpec {
	...
	return bindToolSchemas([]ToolSpec{
		...
		{Name: "private_notes_maintain", Title: "Maintain private notes", Description: "Initialize age encryption, sync encrypted backups, or migrate legacy private-note backups.", Destructive: true, OpenWorld: true, Profiles: unifiedProfiles(), Handler: ctxToolHandler((*Runtime).privateNotesMaintain)},
		...
		{Name: "desktop_clipboard_write", Title: "Write desktop clipboard", Description: "Set macOS clipboard text through a mutating clipboard tool.", Destructive: true, Profiles: unifiedProfiles(), Availability: requiresDesktop, Handler: ctxToolHandler((*Runtime).desktopClipboardWrite)},
