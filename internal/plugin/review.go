...
func buildReview(pkg Package, source Source) Review {
	review := Review{
	...
	review.ReviewToken = makeReviewToken(pkg, source)
...
func makeReviewToken(pkg Package, source Source) string {
...
func verifyReviewToken(pkg Package, source Source, token string) error {
	...
	expected := makeReviewToken(pkg, source)
	if token != expected {
