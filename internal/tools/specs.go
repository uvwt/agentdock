...
func allToolSpecs() []ToolSpec {
	...
	return bindToolSchemas([]ToolSpec{
		{Name: "server_info", Title: "Server info", Description: "Return server, workspace, auth, sandbox, and exposed-tool metadata.", ReadOnly: true, Handler: func(_ context.Context, r *Runtime, _ map[string]any) (Result, error) { return r.serverInfo(), nil }},
		{Name: "read_file", Title: "Read file", Description: "Read a UTF-8 text file slice inside the configured workspace.", ReadOnly: true, Handler: toolHandler((*Runtime).readFile)},
		{Name: "list_dir", Title: "List directory", Description: "List directory entries inside the configured workspace.", ReadOnly: true, Handler: toolHandler((*Runtime).listDir)},
		{Name: "list_files", Title: "List files", Description: "List workspace files using glob and ignore filters.", ReadOnly: true, Handler: toolHandler((*Runtime).listFiles)},
		{Name: "search_text", Title: "Search text", Description: "Search UTF-8 workspace files for text or regex matches.", ReadOnly: true, Handler: toolHandler((*Runtime).searchText)},
		{Name: "workspace_edit", Title: "Edit workspace", Description: "Edit workspace files through one action-based entrypoint: replace, patch, add, delete, or move.", Handler: ctxToolHandler((*Runtime).workspaceEdit)},
		{Name: "exec_command", Title: "Run workspace command", Description: "Run a bounded command in the workspace with sandbox and approval controls.", OpenWorld: true, Handler: ctxToolHandler((*Runtime).execCommand)},
		...
		{Name: "view_image", Title: "View image", Description: "Return a workspace image as MCP image content.", ReadOnly: true, Availability: requiresViewImage, Handler: toolHandler((*Runtime).viewImage)},
