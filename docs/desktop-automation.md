...
- `observe` 的 `snapshot` / `snapshot_app` 默认返回带过期时间的签名公网 `screenshot.url`；只有显式 `return_mode=base64`、`return_mode=data_url`、`return_mode=mcp_image` 或 `return_mode=both` 时才返回内联图片数据。
