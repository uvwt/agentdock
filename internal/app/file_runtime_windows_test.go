...
func TestResolveWSLFilePathAcceptsLinuxAndWindowsDrivePaths(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			got, err := toolfile.ResolveWSLFilePath(test.path)
			if err != nil {
				t.Fatalf("toolfile.ResolveWSLFilePath(%q): %v", test.path, err)
			}
			if got != test.want {
				t.Fatalf("toolfile.ResolveWSLFilePath(%q) = %q, want %q", test.path, got, test.want)
			}
		})
	}
	if _, err := toolfile.ResolveWSLFilePath("relative/path"); err == nil {
...
func TestWSLFileErrorPhaseSeparatesValidationFromRuntime(t *testing.T) {
	if phase := toolfile.WSLFileErrorPhase("SYMLINK_NOT_ALLOWED"); phase != "validation" {
		t.Fatalf("symlink phase = %q", phase)
	}
	if phase := toolfile.WSLFileErrorPhase("WSL_FILE_RUNTIME_ERROR"); phase != "runtime" {
		t.Fatalf("runtime phase = %q", phase)
	}
}

func TestSelectWindowsFileRuntime(t *testing.T) {
	selection, err := toolfile.SelectFileRuntime(map[string]any{})
	if err != nil {
		t.Fatal(err)
	}
	...
	selection, err = toolfile.SelectFileRuntime(map[string]any{"runtime": "wsl", "wsl_distribution": "Ubuntu"})
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := toolfile.SelectFileRuntime(map[string]any{"runtime": "windows", "wsl_distribution": "Ubuntu"}); err == nil {
